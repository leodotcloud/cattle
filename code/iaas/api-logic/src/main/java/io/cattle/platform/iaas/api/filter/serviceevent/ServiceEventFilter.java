...
public class ServiceEventFilter extends AbstractDefaultResourceManagerFilter {
    ...
    public Object create(String type, ApiRequest request, ResourceManager next) {
        ...
        if(!isNetworkService(resourceAccId, healthcheckInstance.getInstanceId())) {
    ...
    private boolean isNetworkService(long accountId, long instanceId) {
        Instance instance = objectManager.findAny(Instance.class, INSTANCE.ID, instanceId);
        List<Service> services = objectManager.find(Service.class, SERVICE.ACCOUNT_ID, accountId, SERVICE.REMOVED, null, SERVICE.STACK_ID,
                instance.getStackId(), SERVICE.KIND, ServiceConstants.KIND_NETWORK_DRIVER_SERVICE);
        if(services.size() > 1) {
            return true;
        }
        return false;
